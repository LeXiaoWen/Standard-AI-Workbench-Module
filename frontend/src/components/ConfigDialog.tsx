...
function isPrivateIpv4(host: string): boolean {
  ...
  const [a, b] = parts;
  // 回环 127/8、RFC1918 私网：10/8、172.16/12、192.168/16
...
const profileSchema = (hasKey: boolean) =>
  z
    .object({
    ...
    .superRefine((values, ctx) => {
      // 已有 key 或指向本机/局域网私网时允许留空，其余（公网）必填
      if (hasKey || values.api_key) return;
      let hostname = "";
      try {
        hostname = new URL(values.base_url).hostname;
      } catch {
        hostname = "";
      }
      if (!isTrustedLocalHost(hostname)) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["api_key"], message: "请输入 API key。" });
      }
    });
