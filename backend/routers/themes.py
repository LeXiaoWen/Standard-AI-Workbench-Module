...
from ..schemas import ThemeActivateRequest, ThemeAppearance, ThemeListResponse, ThemePreferences, UserTheme
...
# 预设皮肤 id（与前端 frontend/src/lib/skins.ts 的 SKINS 保持一致）
VALID_SKIN_IDS = {
    "system", "abyss", "aurora", "nebula", "ember", "midnight", "ivory", "mist", "rose",
}
...
@router.get("/api/v1/themes/preferences", response_model=ThemePreferences)
def get_theme_preferences(request: Request):
    user_id = current_user(request).id
    data = workbench_store.get_theme_preferences(user_id)
    return ThemePreferences.model_validate(data)


@router.patch("/api/v1/themes/preferences", response_model=ThemePreferences)
def update_theme_preferences(request: Request, payload: ThemePreferences):
    user_id = current_user(request).id
    if payload.skin_id not in VALID_SKIN_IDS:
        raise HTTPException(status_code=422, detail="未知皮肤。")
    merged = workbench_store.get_theme_preferences(user_id)
    merged.update(payload.model_dump(exclude_unset=True))
    workbench_store.set_theme_preferences(user_id, merged)
    return ThemePreferences.model_validate(merged)
